...
//    @GetMapping("/searchCompany")
//    public ResponseEntity<Iterable<Company>> findAllByCompanyNameContaining(@RequestParam("companyName") String companyName){
//
//        Iterable<Company> companyIterable = companyService.findAllByCompanyNameContaining(companyName);
//        if (companyIterable == null){
//            return new ResponseEntity<Iterable<Company>>(HttpStatus.NO_CONTENT);
//        }
//        return new ResponseEntity<Iterable<Company>>(companyIterable,HttpStatus.OK);
    ...
    public ResponseEntity<Company> updateUser(@PathVariable("id") Long id, @RequestPart("companyavatar") MultipartFile[] file) throws IOException {
        ...
        if (company != null){
            if (company.getCompanyavatar() != null){
                fileService.deleteImage(company);
            }else
            //lưu tên ảnh
            ...
            company.setCompanyavatar(fileName);
            companyService.save(company);
            return new ResponseEntity<Company>(company,HttpStatus.OK);
        }else {
            for (MultipartFile multipartFile: file){
